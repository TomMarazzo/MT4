...
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
//|Line 522 was ADDED and MUST exist for the Dialog to work properly |
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
//|In the "Stop appliation" section below, | added messages for when |
//|I turn off the dialog box.  It askes for confirmation.            |
//+------------------------------------------------------------------+
...
//--- save parameters
   ...
   m_subwin      =subwin;
   m_program_name=name;
   m_deinit_reason=WRONG_VALUE;//THIS LINE MUST EXIST OR IT SCREWS UP THE ENTIRE DASHBOARD
//--- get unique ID
...
void CAppDialog::OnClickButtonClose(void)
  {
//--- destroy application
   int ans = MessageBox("Close this Dashboard?","Are you sure?",MB_YESNO|MB_ICONQUESTION);
   if(ans == IDNO)return;
   else Destroy();
  }
//+------------------------------------------------------------------+
